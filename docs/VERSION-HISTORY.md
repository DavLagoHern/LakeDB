...
- `FIX` keep focus and the cursor in the JSON cell editor while typing instead
  of moving focus to the Format action after every character.

[Release notes and downloads](https://github.com/DavLagoHern/LakeDB/releases/tag/v1.0.0-beta.6.2.1)
...
- `ADD` infer keyword case, indentation, comma placement and practical clause,
  list, join, predicate and write-statement layouts without AI or external services.
- `CHANGE` point multi-statement failures to the exact SQL while preserving
  completed results.
- `ADD` remember export folders and table column widths, and hide selected
  tables locally in Object Explorer.
...
- `FIX` keep established Enterprise sessions and QuerIA workspaces available
  through temporary service failures, with 5-minute healthy checks and
  30-second recovery checks.
...
- `FIX` Keep LakeDB open while Windows setup runs and confirm when the installer has opened.
- `SECURITY` Preserve SHA-256 verification before exposing the downloaded installer.

[Release notes and downloads](https://github.com/DavLagoHern/LakeDB/releases/tag/v0.11.2-beta.2)
...
- `CHANGE` Publish pre-1.0 betas through the GitHub Latest download URL.
- `SECURITY` Keep the NSIS installer disabled until its matching SHA-256 checksum has been verified.

[Release notes and downloads](https://github.com/DavLagoHern/LakeDB/releases/tag/v0.11.0-beta.2)
